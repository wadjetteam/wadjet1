  ...
  return (
    <div className="h-full overflow-y-auto p-6 space-y-6 scrollbar-thin">
      ...
      <div className="grid grid-cols-4 gap-4 animate-fade-in" style={{ animationDelay: '0.05s' }}>
        {[
          { label: 'Pending Review', value: requests.filter(r => r.status === 'pending').length, color: '#c9a82e' },
          { label: 'Approved', value: requests.filter(r => r.status === 'approved').length, color: '#2d7d46' },
          { label: 'Rejected', value: requests.filter(r => r.status === 'rejected').length, color: '#c41e3a' },
          { label: 'Total Requests', value: requests.length, color: '#b8860b' },
        ].map(stat => (
      ...
      <div className="space-y-4 animate-fade-in" style={{ animationDelay: '0.1s' }}>
        ...
        {filtered.map(req => {
          const severityColor = req.originalScore >= 9 ? '#c41e3a' : req.originalScore >= 7 ? '#c9a82e' : '#b8860b'
