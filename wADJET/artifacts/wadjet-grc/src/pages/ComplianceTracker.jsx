...
import { Shield, CheckCircle, AlertTriangle, FileText, Search, Filter, Clock, Activity } from 'lucide-react'
...
function RiskRow({ risk }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: BORDER, background: 'rgba(212,168,50,0.02)' }}>
      <div onClick={() => setExpanded(!expanded)} className="flex items-center gap-3 p-3 cursor-pointer">
        <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
          style={{ background: `${SEV_COLORS[risk.severity] || SILVER}15` }}>
          <AlertTriangle size={12} style={{ color: SEV_COLORS[risk.severity] || SILVER }} />
        </div>
        ...
        <span className="text-[10px] px-2 py-0.5 rounded-full font-medium"
          style={{ background: `${SEV_COLORS[risk.severity] || SILVER}15`, color: SEV_COLORS[risk.severity] || SILVER, border: `1px solid ${SEV_COLORS[risk.severity] || SILVER}30` }}>
          {risk.severity}
        </span>
...
export default function ComplianceTracker() {
  ...
  const { deptRes, sevPie, statusPie, filtered } = useMemo(() => {
    const deptMap = {}
    const sevCount = {}
    const statusCount = {}
    risks.forEach(r => {
    ...
    const sevPie = Object.entries(sevCount).map(([n, v]) => ({
      name: n, value: v, fill: SEV_COLORS[n] || SILVER
    }))
    const statusPie = Object.entries(statusCount).map(([n, v]) => ({
      name: n, value: v, fill: n === 'Closed' || n === 'Resolved' ? EMERALD : n === 'Open' ? CRIMSON : AMBER
    }))
    ...
    return { deptRes, sevPie, statusPie, filtered }
    ...
    <div className="h-full overflow-y-auto p-6 space-y-5 scrollbar-thin">
      <div className="flex items-center justify-between animate-fade-in">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold" style={{ color: WHITE, fontFamily: "'Inter', sans-serif" }}>Compliance & Audit Tracker</h1>
            <p className="text-xs" style={{ color: GOLD }}>Risk Resolution by Department · Register Browser</p>
          </div>
        </div>
      </div>
          ...
          <div className="p-5 rounded-2xl" style={{ background: BG, border: BORDER }}>
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {filtered.length === 0 ? (
                ...
                filtered.map(r => <RiskRow key={r._id} risk={r} />)
              )}
            </div>
