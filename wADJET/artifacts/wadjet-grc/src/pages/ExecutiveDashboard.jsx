        ...
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: BORDER }}>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px]" style={{ color: GOLD }}>Configure board-level risk appetite & KPI targets</p>
            </div>
          </div>
...
export default function ExecutiveDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const catPie = Object.entries(byCat).sort((a, b) => b[1] - a[1]).slice(0, 6)
      .map(([name, value], i) => {
        const colors = [EMERALD, GOLD, AMBER, CRIMSON, '#6f42c1', '#20c997']
    ...
    <div className="h-full overflow-y-auto p-6 space-y-5 scrollbar-thin">
      ...
      <div className="flex items-center justify-between animate-fade-in">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: GOLD }}>Board-Level Risk Profile · Target-Driven KPIs</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="grid grid-cols-12 gap-5">
            ...
            <div className="flex mb-1" style={{ paddingLeft: '110px' }}>
              {LIKELIHOOD_LABELS.map((l, i) => (
                <div key={i} className="flex-1 text-center">
                  <span className="text-[8px] font-semibold uppercase tracking-wider" style={{ color: 'rgba(206,212,218,0.35)' }}>{l}</span>
                </div>
              ))}
            </div>
        ...
        <div className="col-span-4 space-y-5">
          ...
          {/* Board KPI Gauges with targets */}
          <div className="p-5 rounded-2xl" style={{ background: BG, border: BORDER }}>
            <h2 className="text-sm font-semibold mb-4" style={{ color: WHITE }}>KPI Performance vs Board Targets</h2>
