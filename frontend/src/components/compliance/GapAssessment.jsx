...
const maturityLevels = [
  { level: 1, label: 'Initial', desc: 'Ad-hoc processes', color: '#dc3545' },
  { level: 2, label: 'Repeatable', desc: 'Basic discipline', color: '#fd7e14' },
  { level: 3, label: 'Defined', desc: 'Standardized processes', color: '#d4a832' },
  { level: 4, label: 'Managed', desc: 'Quantitative mgmt', color: '#20c997', target: true },
  { level: 5, label: 'Optimizing', desc: 'Continuous improvement', color: '#20c997' },
]
                ...
                return (
                  <div key={ml.level} className="flex flex-col items-center relative z-10" style={{ width: '18%' }}>
                    ...
                    <div className="mt-2 text-center">
                      <div className={`text-[11px] font-bold tracking-wide ${isCompleted ? 'text-[#d4a832] drop-shadow-[0_0_8px_rgba(212,168,50,0.4)]' : isTarget ? 'text-pharaoh-300' : 'text-pharaoh-500/40'}`}>
                        {ml.label}
                      </div>
        ...
        <div className="bg-zinc-950/80 border border-zinc-800/80 backdrop-blur-xl p-5 rounded-2xl flex flex-col">
          ...
          <div className="flex-1 min-h-0 animate-fade-in" style={{ animationDelay: '0.15s' }}>
            <ResponsiveContainer width="100%" height={200}>
              <RadarChart data={radarData} margin={{ top: 10, right: 20, bottom: 10, left: 20 }}>
                ...
                <PolarAngleAxis dataKey="axis" tick={{ fill: '#ced4da', fontSize: 9, fontWeight: 600, letterSpacing: '0.05em' }} tickLine={false} />
                <PolarRadiusAxis angle={30} domain={[0, 5]} tick={false} axisLine={false} />
                {/* Crimson gap overlay between polygons */}
                <Radar name="Gap Fill" dataKey="gap" stroke="none" fill="#dc3545" fillOpacity={0.08} />
                <Radar name="Target" dataKey="target" stroke="#d4a832" strokeWidth={2.5} fill="none" dot={false} className="transition-all duration-500" style={{ transition: 'all 0.5s cubic-bezier(0.4, 0, 0.2, 1)' }} />
                <Radar name="Current" dataKey="current" stroke="#20c997" strokeWidth={2} fill="#20c997" fillOpacity={0.25} dot={{ r: 3, fill: '#20c997', stroke: '#20c997', strokeWidth: 1 }}
          ...
          <div className="flex items-center justify-center gap-4 mt-2">
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md border border-amber-500/30 bg-amber-500/5">
              <div className="w-2 h-2 rounded-full bg-[#d4a832]" />
