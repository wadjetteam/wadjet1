...
const riskColors = { low: '#20c997', medium: '#d4a832', high: '#fd7e14', critical: '#dc3545' }
const statusCfg = {
  active: { label: 'Active', color: '#20c997' },
  review: { label: 'Under Review', color: '#d4a832' },
  remediation: { label: 'Remediation Required', color: '#dc3545' },
  offboarding: { label: 'Offboarding', color: '#ced4da' },
}
...
function ConcentrationBar({ value, name }) {
  const color = value >= 80 ? '#dc3545' : value >= 60 ? '#d4a832' : '#20c997'
  return (
  ...
  return (
    <div className="h-full overflow-y-auto p-6 space-y-6 scrollbar-thin">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: 'Total Vendors', value: vendors.length, sub: `${vendors.filter(v => v.tier === 1).length} Tier 1 critical`, color: '#d4a832' },
          { label: 'High Risk Vendors', value: highRisk, sub: 'Require urgent action', color: '#dc3545' },
          { label: 'Contracts Expiring (6M)', value: expiringSoon, sub: 'Renewal action needed', color: '#fd7e14' },
          { label: 'Annual Spend (M EGP)', value: `${totalSpend.toFixed(1)}`, sub: 'Under TPRM management', color: '#20c997' },
        ].map(k => (
        ...
        <div className="pharaoh-card p-5">
          ...
          <ResponsiveContainer width="100%" height={170}>
            <BarChart data={vendors.filter(v => v.openFindings > 0)} layout="vertical" margin={{ left: 0, right: 20 }}>
              <XAxis type="number" tick={{ fill: 'rgba(212,168,50,0.5)', fontSize: 10 }} axisLine={false} tickLine={false} />
              <YAxis type="category" dataKey="name" tick={{ fill: 'rgba(212,168,50,0.5)', fontSize: 9 }} axisLine={false} tickLine={false} width={100}
      ...
      <div className="flex items-center gap-2">
        ...
        {['all', '1', '2'].map(t => (
          <button key={t} onClick={() => setFilterTier(t)}
            ...
            style={filterTier !== t ? { background: 'rgba(212,168,50,0.05)', border: '1px solid rgba(212,168,50,0.1)' } : {}}>
        ...
        {['all', 'low', 'medium', 'high'].map(r => (
          <button key={r} onClick={() => setFilterRisk(r)}
            ...
            style={filterRisk !== r ? { background: 'rgba(212,168,50,0.05)', border: '1px solid rgba(212,168,50,0.1)' } : {}}>
            {r}
          </button>
        ))}
      </div>
        ...
        {filtered.map(v => {
          ...
          return (
            <div key={v.id} className="pharaoh-card overflow-hidden">
              <div className="p-4 flex items-center gap-4 cursor-pointer" onClick={() => setExpanded(isExpanded ? null : v.id)}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded" style={{ background: 'rgba(212,168,50,0.08)', color: '#d4a832' }}>Tier {v.tier}</span>
                ...
                <div className="flex items-center gap-4 flex-shrink-0">
                  <div className="text-right">
                    <p className="text-xs font-semibold" style={{ color: daysToExpiry <= 90 ? '#dc3545' : daysToExpiry <= 180 ? '#fd7e14' : '#d4a832' }}>
              ...
              {isExpanded && (
                <div className="px-5 pb-5 border-t animate-fade-in" style={{ borderColor: 'rgba(212,168,50,0.1)' }}>
                  <div className="grid grid-cols-2 gap-6 mt-4">
                    <div>
                      ...
                      <ResponsiveContainer width="100%" height={190}>
                        <RadarChart data={radarData} margin={{ top: 8, right: 22, bottom: 8, left: 22 }}>
                          ...
                          <PolarGrid stroke="rgba(212,168,50,0.12)" gridType="polygon" />
                          <PolarAngleAxis
                            dataKey="subject"
                            tick={{ fill: 'rgba(212,168,50,0.7)', fontSize: 10, fontWeight: 600 }}
                            tickLine={false}
                          />
                          <PolarRadiusAxis
                            angle={90} domain={[0, 5]} tickCount={4}
                            tick={{ fill: 'rgba(212,168,50,0.2)', fontSize: 8 }}
                            axisLine={false}
                          />
                          <Radar name="Benchmark" dataKey="fullMark"
                            stroke="rgba(206,212,218,0.3)" strokeWidth={1} strokeDasharray="3 3"
                            fill="rgba(206,212,218,0.05)" dot={false} />
                          <Radar name="Score" dataKey="value"
                            stroke={riskColors[v.riskLevel]} strokeWidth={2.5}
                            fill={`url(#radarVendor${v.id})`}
                            dot={{ r: 3.5, fill: riskColors[v.riskLevel], stroke: '#060d15', strokeWidth: 1.5,
                              style: { filter: `drop-shadow(0 0 4px ${riskColors[v.riskLevel]}80)` } }} />
                          <Tooltip contentStyle={{ background: 'rgba(8,18,34,0.97)', border: '1px solid rgba(212,168,50,0.2)', borderRadius: 10, fontSize: 11 }}
                            labelStyle={{ color: '#d4a832', fontWeight: 600 }} />
                        </RadarChart>
                      </ResponsiveContainer>
                      <div className="flex items-center justify-center gap-4 mt-1">
                        ...
                        <div className="flex items-center gap-1.5">
                          <span className="w-4 h-0.5 rounded-full inline-block" style={{ borderTop: '1px dashed rgba(206,212,218,0.5)' }} />
                    ...
                    <div className="space-y-3 pt-2">
                      <div className="grid grid-cols-2 gap-2">
                        <div className="p-2 rounded-xl text-center" style={{ background: 'rgba(212,168,50,0.05)', border: '1px solid rgba(212,168,50,0.1)' }}>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span style={{ color: v.dataAccess ? '#dc3545' : '#20c997' }}>{v.dataAccess ? 'Yes — PII/Financial' : 'No'}</span>
                        </div>
                      ...
                      <button className="w-full py-2 rounded-xl text-xs font-medium transition-all" style={{ background: 'rgba(212,168,50,0.08)', border: '1px solid rgba(212,168,50,0.2)', color: '#d4a832' }}>
