...
js_path = r'C:\Users\Admin\Desktop\واجيت\wADJET\artifacts\wadjet-grc\src\components\PolicyManagement.jsx'

with open(js_path, 'r', encoding='utf-8') as f:
    content = f.read()

full_text = """[THE BANK LOGO / NAME]
...
# We need to replace everything between content:` and the closing ` },{id:pid(2)
pattern = r"(content:\`)(.*?)(\")\s*\},\s*\{id:pid\(2\)"
replacement = r"\1" + escaped + r"\3" + "},\n    {id:pid(2)"

# Actually, let's try a different approach. Find the exact start and end positions.
...
if start_idx == -1 or end_idx == -1:
    ...
    new_content = content[:start_idx + len(start_marker)] + escaped + content[end_idx:]
    with open(js_path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print(f"Replaced successfully. New file length: {len(new_content)} chars")
