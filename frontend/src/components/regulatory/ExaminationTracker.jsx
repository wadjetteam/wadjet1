...
const findingTypeColors = { MRA: '#dc3545', MRIA: '#d4a832', Requirement: '#d4a832', Observation: '#ced4da', 'Action Point': '#20c997' }
const severityColors = { critical: '#dc3545', high: '#d4a832', medium: '#d4a832', low: '#20c997' }
const statusColors = { open: '#dc3545', 'in-progress': '#d4a832', closed: '#20c997' }
  ...
  return (
    <div className="h-full overflow-y-auto p-6 space-y-6 scrollbar-thin">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: 'Total Open Findings', value: openCount, color: '#d4a832', sub: `of ${findings.length} total` },
          { label: 'Overdue Remediation', value: overdueCount, color: '#dc3545', sub: 'Past due date' },
          { label: 'Critical Open (MRA)', value: criticalOpen, color: '#dc3545', sub: 'Immediate action required' },
          { label: 'Avg. Days Open', value: Math.round(findings.filter(f => f.status !== 'closed').reduce((s, f) => s + f.agingDays, 0) / openCount), color: '#fd7e14', sub: 'Open findings' },
        ].map(k => (
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="pharaoh-card p-4 col-span-1">
          ...
          <ResponsiveContainer width="100%" height={140}>
            ...
            <XAxis dataKey="range" tick={{ fill: 'rgba(212,168,50,0.5)', fontSize: 10 }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: 'rgba(212,168,50,0.5)', fontSize: 10 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ background: '#060d15', border: '1px solid rgba(212,168,50,0.2)', borderRadius: 8, fontSize: 11 }} />
          ...
          <div className="space-y-2">
            {examinations.map(ex => (
              <button key={ex.id} onClick={() => setSelectedExam(selectedExam === ex.id ? 'all' : ex.id)}
                ...
                style={selectedExam !== ex.id ? { background: 'rgba(212,168,50,0.04)', border: '1px solid rgba(212,168,50,0.1)' } : {}}>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {['all', 'open', 'in-progress', 'closed'].map(s => (
          <button key={s} onClick={() => setFilterStatus(s)}
            ...
            style={filterStatus !== s ? { background: 'rgba(212,168,50,0.05)', border: '1px solid rgba(212,168,50,0.1)' } : {}}>
            {s.replace('-', ' ')}
          </button>
        ))}
      </div>
        ...
        {filtered.map(f => {
          ...
          return (
            <div key={f.id} className="pharaoh-card overflow-hidden">
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  <div className="w-20">
                    <div className="flex justify-between text-[10px] mb-1">
                      ...
                      <span style={{ color: f.progressPct === 100 ? '#20c997' : '#d4a832' }}>{f.progressPct}%</span>
                    </div>
                    <div className="h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
                      <div className="h-full rounded-full transition-all" style={{ width: `${f.progressPct}%`, background: f.progressPct === 100 ? '#20c997' : '#d4a832' }} />
                    </div>
                  </div>
              ...
              {isExpanded && (
                <div className="px-5 pb-5 border-t animate-fade-in space-y-3" style={{ borderColor: 'rgba(212,168,50,0.1)' }}>
                  <div className="mt-4">
                  ...
                  <div className="flex gap-2 pt-1">
                    <button className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all" style={{ background: 'rgba(212,168,50,0.08)', border: '1px solid rgba(212,168,50,0.2)', color: '#d4a832' }}>
                      Update Progress
                    </button>
                    {f.status !== 'closed' && (
