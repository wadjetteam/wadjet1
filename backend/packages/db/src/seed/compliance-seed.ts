...
async function seedISOControls() {
...
async function seedPCIControls() {
...
async function seedCBEControls() {
  for (const c of CBE_CONTROLS) {
    await Control.create({
...
async function seedCrossMappings() {
  for (const m of ISO_CBE_PCI_MAPPINGS) {
    await CrossMapping.create({ ...m, createdAt: new Date().toISOString() });
  }
}

const firstRef = (s: string) => s.split(" / ")[0].trim();

function cbeToDb(ref: string): string {
  if (ref === "—" || !ref) return "";
...
function pciToDb(ref: string): string {
  if (ref === "—" || !ref) return "";
  const r = firstRef(ref);
  return r.split(".")[0];
}

function isoToDb(ref: string): string {
  let r = ref;
...
async function seedTripleMappings() {
  const existing = await TripleMapping.find().lean();
...
export async function seedComplianceData() {
  ...
  await seedDomains();
  await seedISOControls();
  await seedPCIControls();
  await seedCBEControls();
  await seedCrossMappings();
  await seedTripleMappings();

  console.log(
    `Seeded ${FRAMEWORKS.length} frameworks, ${DOMAINS.length} domains, ${
      ISO_CONTROLS.length + PCI_CONTROLS.length + CBE_CONTROLS.length
    } controls, ${ISO_CBE_PCI_MAPPINGS.length} cross-mappings, ${TRIPLE_MAPPINGS.length} triple mappings`
  );
}
